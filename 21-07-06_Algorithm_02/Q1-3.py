...
def correctLetterOrder(saleKind, saleNumber, products): # 판매된 물품 코드 문자열 배열, 판매된 물품의 개수, 편의점에서 판매하는 물품 코드 배열 (13종 고정)
    ...
    saleKind = list(map(lambda x : ''.join(sorted(list(x))),saleKind))
    products_tmp = list(map(lambda x : ''.join(sorted(list(x))),products))
    for i in saleKind:
        idx.append(products_tmp.index(i))
